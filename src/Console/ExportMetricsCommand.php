...
final class ExportMetricsCommand extends Command
{
    ...
    public function handle(QueuesData $queues, MetricsRepository $metrics): int
    {
        ...
        $this->printScalarGauge(
            'zenith_outbox_backlog',
            'Outbox rows waiting to be relayed onto the real queue.',
            $backlog['count'],
        );

        $this->printScalarGauge(
            'zenith_outbox_oldest_pending_seconds',
    ...
    private function printScalarGauge(string $name, string $help, ?int $value): void
    {
        if ($value === null) {
            return;
        }
