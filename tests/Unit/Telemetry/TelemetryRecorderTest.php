...
it('increments a bounded, fixed number of redis fields per recorded attempt', function (): void {
    ...
    // 3 resolutions x 3 dimensions x (1 count + 1 runtime histogram + 1 wait histogram) field.
    expect($connection->callCounts['hincrby'] ?? 0)->toBe(27);
...
it('skips histogram fields but still records the outcome when timings are unavailable', function (): void {
    ...
    // 3 resolutions x 3 dimensions x 1 count field only.
    expect($connection->callCounts['hincrby'] ?? 0)->toBe(9);
