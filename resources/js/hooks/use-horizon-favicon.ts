...
const runningColor = "#10b981";
const attentionColor = "#f59e0b";

function faviconForStatus(status: HorizonStatus): string {
  const indicatorColor = status === "running" ? runningColor : attentionColor;
  const svg = [
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 30 30">',
    `<path fill="#7746ec" d="${horizonMarkPath}"/>`,
    '<circle cx="24" cy="24" r="5.5" fill="#fff"/>',
    `<circle cx="24" cy="24" r="4" fill="${indicatorColor}"/>`,
    "</svg>",
  ].join("");
...
export function useHorizonFavicon(status: HorizonStatus) {
  useEffect(() => {
    const favicon = document.querySelector<HTMLLinkElement>("[data-horizon-favicon]");

    if (favicon === null) {
      return;
    }

    favicon.dataset.horizonStatus = status;
    favicon.href = faviconForStatus(status);
  }, [status]);
}
